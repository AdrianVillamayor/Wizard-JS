...
function f(e, t) {
	return !t && !e.required ? !0 : e.type === "checkbox" || e.type === "radio" ? e.checked : typeof e.value == "string" && e.value.trim() === "" ? !1 : typeof e.checkValidity == "function" ? e.checkValidity() : !0;
}
function p(e, t, n) {
	let r = !1, i = [];
	return t.forEach((t) => {
		...
		f(t, a || o) || (r = !0, i.push(t), n.highlight && n.highlightElement(t, n.highlight_type.error));
	}), {
		error: r,
		target: i
	};
...
var m = class {
	...
	init() {
		...
		this.root = e, this.form = e.tagName === "FORM", this.decorateStructuralClasses(e), e.classList.add(this.wz_ori.replace(".", "")), this.checkAndPrepare(e), this.bindEvents(e), e.style.display = e.classList.contains("vertical") ? "flex" : "block", e.setAttribute("data-wz-load", "true"), document.dispatchEvent(new CustomEvent("wz.ready", {
	...
	updateToForm() {
		...
		if (t.tagName !== "FORM") {
			let n = t.getAttribute("class"), r = t.innerHTML;
			t.remove();
			let i = document.createElement("form");
			i.setAttribute("method", "POST"), i.setAttribute("class", `${n} ${this.wz_form.replace(".", "")}`), i.innerHTML = r, e.appendChild(i), this.decorateStructuralClasses(e);
		}
	}
	checkForm() {
		...
		return n.length === 0 ? { error: !1 } : p(e, n, {
	...
	onClick(e) {
		...
		if (this.locked && this.locked_step === this.getCurrentStep()) {
			t.dispatchEvent(new Event("wz.lock", { bubbles: this.bubbles }));
			return;
		}
		let n = e.closest(this.wz_class) || t, i = e.classList.contains(r(this.wz_button) || ""), a = e.classList.contains(r(this.wz_step) || ""), o = e.getAttribute("data-wz-step");
		o = o === null ? this.getCurrentStep() : parseInt(o, 10), i && (e.classList.contains(r(this.wz_prev) || "") ? (--o, t.dispatchEvent(new Event("wz.btn.prev", { bubbles: this.bubbles }))) : e.classList.contains(r(this.wz_next) || "") && (o += 1, t.dispatchEvent(new Event("wz.btn.next", { bubbles: this.bubbles }))));
		let s = o > this.getCurrentStep();
		if (a && (s ? t.dispatchEvent(new Event("wz.nav.forward", { bubbles: this.bubbles })) : o < this.getCurrentStep() && t.dispatchEvent(new Event("wz.nav.backward", { bubbles: this.bubbles }))), this.form && this.navigation !== "buttons" && s && o !== this.getCurrentStep() + 1 && (o = o >= this.last_step ? this.last_step : this.getCurrentStep() + 1), this.form) {
			let e = this.checkForm();
			if (e.error && (s && t.dispatchEvent(new CustomEvent("wz.error", {
				bubbles: this.bubbles,
				detail: {
					id: "form_validation",
			...
			})), this.last_step = this.getCurrentStep(), this.getCurrentStep() < o)) return;
		}
		o != null && this.setCurrentStep(o), this.syncUI(n);
	}
	onClickFinish() {
		let e = this.getRoot();
		if (this.form) {
	...
	setStep(e) {
		let t = o(Number(e), this.steps || this.getTotalSteps());
		return this.last_step = Math.max(t, o(this.last_step, this.steps || this.getTotalSteps())), t;
	}
	setNavEvent() {
	...
	setBtnEvent() {
		this.getRoot().addEventListener("click", (e) => {
			...
			t && (e.preventDefault(), t.classList.contains(r(this.wz_finish) || "") ? this.onClickFinish() : this.onClick(t));
